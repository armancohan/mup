...
def get_optimizer(model, config):
    ...
    if optim_name.lower() == "adam":
        ...
        optimizer = optim.SGD(param_groups, lr=config.learning_rate, weight_decay=config.weight_decay)
    elif optim_name.lower() == "adamw":
        optimizer = optim.AdamW(param_groups, lr=config.learning_rate, weight_decay=config.weight_decay, eps=1e-8)
